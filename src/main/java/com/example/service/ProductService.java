...
public class ProductService {
    ...
    public ProductDto.ProductPageResponse listProducts(int page, int size) {
        ...
        List<ProductDto.ProductResponse> products = productRepository.findAllActive(page, effectiveSize).stream()
                .map(productMapper::toResponse)
                .toList();
    ...
    public ProductDto.ProductResponse updateProduct(UUID id, ProductDto.UpdateProductRequest request) {
        Product product =
                productRepository.findByIdOptional(id).orElseThrow(() -> new ResourceNotFoundException("Product", id));
