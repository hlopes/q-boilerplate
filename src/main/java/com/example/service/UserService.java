 ...
 * Handles CRUD, validation, caching, and password hashing.
 */
@ApplicationScoped
...
public class UserService {
    ...
    public UserDto.UserPageResponse listUsers(int page, int size) {
        ...
        List<UserDto.UserResponse> users = userRepository
                .findAllActive(page, effectiveSize)
                .stream()
    ...
    public List<UserDto.UserResponse> searchUsers(String keyword, int page, int size) {
        int effectiveSize = Math.min(size, appConfig.pagination().maxPageSize());
        return userRepository.search(keyword, page, effectiveSize)
                .stream()
                .map(userMapper::toResponse)
                .toList();
    }
    ...
    /**
     * Placeholder for a real password hashing implementation.
     * In production, use bcrypt via quarkus-elytron-security-properties-file
     * or Argon2 via a dedicated library.
     *
