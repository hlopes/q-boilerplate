...
/**
 * Repository for {@link User} data access operations. Uses Panache repository pattern for clean
 * data access layer.
 */
@ApplicationScoped
public class UserRepository implements PanacheRepositoryBase<User, UUID> {
    ...
    public List<User> findByRole(User.UserRole role, int page, int size) {
        return find("role", Sort.by("email"), role).page(Page.of(page, size)).list();
    }

    /**
     * Searches users by email containing the given keyword.
     *
    ...
    public List<User> search(String keyword, int page, int size) {
        ...
        return find("lower(email) like ?1", Sort.by("email"), likePattern)
                .page(Page.of(page, size))
                .list();
    }
