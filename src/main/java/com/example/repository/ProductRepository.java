...
public class ProductRepository implements PanacheRepository<Product> {
    ...
    public List<Product> findByCategory(String category, int page, int size) {
        return find("category = ?1 and status = ?2",
                Sort.by("name"), category, Product.ProductStatus.ACTIVE)
                .page(Page.of(page, size))
                .list();
    }
    ...
    public List<Product> findByPriceRange(BigDecimal minPrice, BigDecimal maxPrice, int page, int size) {
        return find("price >= ?1 and price <= ?2 and status = ?3",
                Sort.by("price"), minPrice, maxPrice, Product.ProductStatus.ACTIVE)
                .page(Page.of(page, size))
                .list();
    }
    ...
    public List<Product> findLowStock(int threshold) {
        return find("stockQuantity < ?1 and status = ?2",
                threshold, Product.ProductStatus.ACTIVE).list();
    }
