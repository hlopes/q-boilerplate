...
public class UserResource {
    ...
    public Response listUsers(
        ...
        UserDto.UserPageResponse result = userService.listUsers(
                page, Math.min(size, appConfig.pagination().maxPageSize()));
    ...
    @Path("/search")
    @RolesAllowed({"ADMIN", "MODERATOR"})
    @Operation(summary = "Search users", description = "Searches users by email keyword")
    @APIResponse(
            responseCode = "200",
    ...
    public Response getUserById(@Parameter(description = "User ID", required = true) @PathParam("id") UUID id) {
    ...
    public Response createUser(@Valid UserDto.CreateUserRequest request, @Context UriInfo uriInfo) {
        ...
        URI location =
                uriInfo.getAbsolutePathBuilder().path(created.id().toString()).build();
